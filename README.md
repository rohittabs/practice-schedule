...
> **Your practice thingy.** A free, beautiful practice tracker app for guitar and ukulele.

No accounts, no subscriptions, no app store. Just one file that works on every device.

---

## What is this?

Practice Schedule is a web app that lives in a single HTML file. Open a link in a browser, add it to a home screen like a real app, and use it every day to track practice. All data is saved privately on that device only.

**Live link:** https://rohittabs.github.io/practice-schedule
...
| **Timer** | Built-in stopwatch that auto-stops at the target time with a celebration screen, then offers to log the time or keep practicing |
| **Metronome** | A simple BPM-only metronome (40 to 240 bpm) appears inside the practice timer. One continuous click, no accents, no beat or time-signature settings. Slider plus quick plus/minus buttons to change tempo |
| **Extra practice** | Log anything beyond the plan for today. It gets its own name, category, and target minutes, shows inline in the Today list with an EXTRA tag, and gets its own timer and tick, just like a routine item |
...
| **Summary export** | One-tap copy of a WhatsApp-formatted bold summary, ready to paste and send |
...
Open this link in a browser: **https://rohittabs.github.io/practice-schedule**

Any modern browser works. Below are the exact steps for the two most common browsers on each device, so pick whichever is already installed.

### iPhone or iPad

**Using Safari (recommended for iOS):**
...
5. An app icon appears on the home screen. Always open the app from this icon afterward
...
> Safari is the more reliable option on iPhone and iPad for saving data long term. If Chrome is preferred, that is fine too, but if anything ever looks like it forgot data, try reopening through Safari instead.

### Android phone or tablet

**Using Chrome (default on most Android devices):**
1. Open **Chrome** and go to the link above
...
**Using other browsers (Samsung Internet, etc.):**
...
**Using Edge (built into Windows):**
1. Open **Edge** and go to the link above
2. Click the **three dots** menu, then **Apps**, then **Install this site as an app**
3. Click **Install**

Bookmarking also works on either browser: press **Ctrl + D** to save a quick shortcut without installing.
...
**Using Chrome:**
1. Open **Chrome** and go to the link above
2. Click the **install icon** in the address bar (or the three-dot menu, then **Cast, save, and share**, then **Install page as app**)
3. Click **Install**
...
- Tap the **clock icon** to use the built-in timer. It stops automatically at the target time and offers to log the time or keep going
...
- All practice data is stored only on the device it is used on
- Nothing is sent to any server
- Data is never seen by anyone else unless the summary is shared directly
...
This usually means it was opened from a file preview rather than a browser. Open Safari, go to the link, and use Add to Home Screen.
...
- Single self-contained HTML file, roughly 245 KB
...
- Tested with 153 automated tests (46 logic tests, 107 end-to-end tests)
- Compatible with Chrome, Edge, Safari, and Firefox on all platforms

---

*Made with love for anyone learning an instrument.*
